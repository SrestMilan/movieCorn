...
function MainComponent({ movies }) {
  return (
    <main className="main">
      <FirstBoxComponent movies={movies} />
      <SecondBoxComponent />
    </main>
  );
}

function FirstBoxComponent({ movies }) {
  ...
  return (
    <div className="box">
      ...
      {isOpen1 && <ListMovie movies={movies} />}
    </div>
  );
}
...
function WatchMovieList({ watched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <MovieRating movie={movie} key={movie.imdbID} />
      ))}
    </ul>
  );
}
